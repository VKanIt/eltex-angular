...
@Injectable()
export class BlogsRepositoryLc implements IBlogsRepository {
    //-----INJECTS-----\\
    ...
    public addBlog(blog: Blog): Observable<Blog[]> {
        return new Observable((subscriber) => {
            ...
            setTimeout(() => {
                let data = this.storageService.loadInStorage('blogs');
                
                blog.id = (data.length > 0 ? data.reduce((max, val) => Math.max(max, val.id), 1) : 0) + 1;

                data.unshift(blog);
                localStorage.setItem('blogs', JSON.stringify(data));
                data = data.slice(0, this.blogsStore.limitActive());
    ...
    public editBlog(newData: {title: string, text: string, image: File|null}, index: number): Observable<Blog[]> {
        return new Observable((subscriber) => {
            this.blogsStore.isDisabled.set(true);
